...
const HEADER_NAME_CHECKSUM_SHA1 = 'TLDR-Checksum-SHA1';
const HTTP_RESPONSE_CODE_FOUND = 302;
const PAUSE_BEFORE_DELETE_BAD_DOWNLOAD_MS = 10000;
...
var httpResponseObserver =
{
    observe : function(subject, topic, data)
    {
        ...
        if (channel.responseStatus != HTTP_RESPONSE_CODE_FOUND) return;
        ...
        try
        {
            ...
            sha1 = channel.getResponseHeader(HEADER_NAME_CHECKSUM_SHA1);
        }
        catch (e)
        {
        ...
        console.log('Received a TLDR header during an '+HTTP_RESPONSE_CODE_FOUND+' HTTP response');
        console.log(HEADER_NAME_CHECKSUM_SHA1+':', sha1);
        ...
        onDownloadChanged: download =>
        {
            if (download.succeeded && download.stopped)
            {
                ...
                */
                ...
                if (download.source.referrer && tldr[download.source.referrer])
                {
                    link = download.source.referrer;
                }
                else if (tldr[download.source.url])
                {
                ...
                if (link && tldr[link])
                {
                    console.log("SHA1 from HTTP Header:", tldr[link].sha1);

                    console.time('computeSHA1');
                    var sha1 = computeSHA1(download.target.path);
                    console.timeEnd('computeSHA1');

                    console.log("SHA1 calculated result: ", sha1);

                    if (sha1 == tldr[link].sha1)
                    {
                    ...
                    else
                    {
                        ...
                        console.log("panel created. About to use panel.on(). Here is tldr[link]", tldr[link]);

                        // this is necessary as the panel.on('show') function cannot read tldr[link]
                        var trustedLinker = tldr[link].trustedLinker;
                        var trustedLinkerReferrer = tldr[link].trustedLinkerReferrer;
                        ...
                        console.log("Will remove downloaded file in "+PAUSE_BEFORE_DELETE_BAD_DOWNLOAD_MS+" milliseconds...");
                        ...
                        try
                        {
                            ...
                            }, PAUSE_BEFORE_DELETE_BAD_DOWNLOAD_MS, download.target.path);
                        }
                        catch (e)
                        {
                            console.error(e);
                    ...
                    delete tldr[link];
                    console.log("Purged "+link+" from tldr object.  It now looks like this:", tldr);
                    console.log('----------------');
                }
            }
        },
